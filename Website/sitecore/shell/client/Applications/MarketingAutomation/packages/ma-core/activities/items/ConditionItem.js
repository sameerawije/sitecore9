...
import { LINE_RADIUS, CONDITION_HEADER_SIZE, CONDITION_WITH_DECISION_POINT_HEADER_SIZE, CONDITION_FOOTER_SIZE, CONDITION_HSPACING, CONDITION_HANDLE_HEIGHT, CONDITION_CONNECTOR_HEIGHT_DELTA } from '../constants';
import { TranslateService } from '@ngx-translate/core';
var ConditionItem = (function (_super) {
    __extends(ConditionItem, _super);
    function ConditionItem(itemData, root, parent) {
        var _this = this;
        if (itemData.showLabel === undefined) {
            itemData.showLabel = true;
        }
        _this = _super.call(this, itemData, root, parent) || this;
        if (_this.children) {
    ...
    Object.defineProperty(ConditionItem.prototype, "hasLabel", {
        get: function () {
            return this.params.showLabel ? true : false;
        },
        enumerable: true,
        configurable: true
    });
    ...
    ConditionItem.prototype.computeChildrenOffsets = function () {
        ...
        var height = 0, width = 0, maxChildWidth = 0, dx;
        this.children.forEach(function (child) {
            if (child.hasDecisionPoint) {
                var childDecisionPoint = child.children[0];
                child.position.offsetX = width;
                child.position.offsetY = _this.visual.height + LINE_RADIUS - childDecisionPoint.visual.dx;
                width += childDecisionPoint.yesSequence.position.width + CONDITION_HSPACING;
            }
            else {
        ...
        dx = (this.noSequence.position.offsetX + this.noSequence.position.dx +
            this.yesSequence.position.offsetX + this.yesSequence.position.dx) / 2;
    ...
    ConditionItem.prototype.getConnectors = function (resultArray) {
        ...
        var x1 = this.visual.x + this.visual.dx, hy = this.visual.y + this.visual.height, fy = this.position.y + this.position.height;
        var idx = 0;
        this.children.forEach(function (child) {
            var pos = child.position, x2 = pos.x + pos.dx, y1 = pos.y, y2 = pos.y + pos.height;
            ...
            if (!(child.hasFinal || child.hasDecisionPoint)) {

  ...
  function (Sitecore, ExperienceEditorContext, ExperienceEditorProxy, TranslationUtil) {
    ...
    var experienceEditor = {
      ...
      getPageDatasourcesItemIDs: function (context, func) {
        ...
        var itemIDs = [];
        experienceEditor.PipelinesUtil.generateRequestProcessor("ExperienceEditor.Datasources.GetDatasourceUsagesWithVersions", function (response) {
      ...
      isShowDatasourcesIsTicked: function () {
        ...
        var showDataSourcesCheckControl = experienceEditor.CommandsUtil.getControlsByCommand(experienceEditor.RibbonApp.getApp().Controls, "ShowDataSources")[0];
      ...
      isInMode: function (mode) {
        return experienceEditor.RibbonApp.getApp().currentContext.webEditMode.toLowerCase() == mode.toLowerCase();
      },
      ...
      canToggleDebug: function () {
        if (experienceEditor.canDebug == null) {
            experienceEditor.canDebug = experienceEditor.RibbonApp.getApp().canExecute("ExperienceEditor.ToggleDebugRequests.CanToggleDebug", experienceEditor.RibbonApp.getApp().currentContext);
        }
      ...
      isInSharedLayout: function (context) {
        var editAllVersionsControls = this.CommandsUtil.getControlsByCommand(experienceEditor.RibbonApp.getApp().Controls, "SelectLayout");
      ...
      isEditingAndDesigningAllowed: function () {
        ...
        if (!isAllowed) {
          ...
          for (var i = 0; i < commands.length; i++) {
            var controls = this.CommandsUtil.getControlsByCommand(experienceEditor.RibbonApp.getApp().Controls, commands[i]);
      ...
      setSaveButtonState: function (isEnabled) {
          var saveButtonControls = experienceEditor.CommandsUtil.getControlsByCommand(experienceEditor.RibbonApp.getApp().Controls, "Save");
      ...
      refreshOnItem: function (context, allowParameterRedirection, keepVersion, keepSite) {
        experienceEditor.modifiedHandling(null, function (isOk) {
          if (allowParameterRedirection ||
            escape(experienceEditor.RibbonApp.getApp().currentContext.itemId) != escape(context.itemId)) {
    ...
    experienceEditor.Common = {
      ...
      closeFullContentIframe: function (iframe) {
        if (!iframe && !ExperienceEditorContext.openedFullContentIframe
          && window.frameElement && window.frameElement.className === "scGalleryFrame") {
      ...
      displayTab: function (tabControl) {
        ...
        if (experienceEditor.RibbonApp.getApp() && experienceEditor.RibbonApp.getApp().cachedCommands) {
            experienceEditor.CommandsUtil.runCommandsCollectionCanExecute(experienceEditor.RibbonApp.getApp().cachedCommands, function (stripId) {
    ...
    experienceEditor.PipelinesUtil = {
      ...
      generateRequestProcessor: function (requestType, onSuccess, customContext, async) {
        var pipelineProcessor =
        {
          ...
          execute: function (context) {
            ...
            experienceEditor.Web.postServerRequest(requestType, postContext, function (response) {
              ...
              if (response.responseValue.abortMessage
                ...
                experienceEditor.Dialogs.alert(response.responseValue.abortMessage, function () {
                  context.abort();
                });
                context.suspend();
                return;
      ...
      initAndExecutePipeline: function(pipeline, context, completion) {
        experienceEditor.PipelinesUtil.initPipeline(pipeline, function () {
          experienceEditor.PipelinesUtil.executeProcessors(Sitecore.Pipelines[pipeline.get("pipelineName")], context, completion);
        });
      }
    };
    ...
    experienceEditor.CommandsUtil = {
      ...
      getSitecoreCommands: function () {
        return !Sitecore.Speak ? Sitecore.Commands : experienceEditor.RibbonApp.getRibbonWindow().Sitecore.Speak.Commands;
      },
      ...
      executeButtonCommand: function (button) {
        console.warn("Obsolete. This method is no longer supported and will be deleted in the next product version.");
      ...
      executeCommand: function (commandName, commandArgument, context) {
        ...
        experienceEditor.CommandsUtil.getSitecoreCommands().executeCommand(this.formCommandName(commandName), context);
      },
      ...
      getCommand: function (commandName) {
        ...
        try {
          return experienceEditor.CommandsUtil.getSitecoreCommands().getCommand(this.formCommandName(commandName));
        } catch (err) {
          return null;
        }
      },
